...
class AlignerTrainer(FCDistanceTrainer):
    ...
    def init(self, input_size, model_params, lr, device):
        ...
        self.model = RuneAligner(input_size=input_size, **model_params).to(self.device)
        self.margin = 2

        self.criterion = lambda anchor_pos, anchor_neg: (anchor_neg - anchor_pos - self.margin).clamp_()
